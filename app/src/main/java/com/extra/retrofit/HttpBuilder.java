...
public class HttpBuilder {
    ...
    public HttpBuilder postBody(String bodyStr) {
    ...
    public  void postSoap(){
        if (!allready())return;

        Call  call = HttpUtil.getService().soap(checkHeaders(headers),checkUrl(this.url),bodyStr);
        putCall(tag,url,call);
        ...
        call.enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) {
                if (response.code() ==200)
                    mSuccessCallBack.Success((String) response.body());
                else mErrorCallBack.Error(message(response.message()));

                if (tag != null) HttpUtil.removeCall(url);
            }
